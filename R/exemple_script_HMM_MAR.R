...
transition_matrix <- matrix(c(0.99, 0.01,
                              0.1, 0.9), byrow = TRUE,
                            nrow = 2)
simulated_tibble <- simulate_HMM_MAR(10000, Y0 = c(-10, -10, 0, 0),
...
#                                  set_ar_from_ou,
#                                  delta_t = 0.5)
# start_trans_mat <- matrix(c(0.8, 0.2, 0.2, 0.8), nrow = 2)

estimation <- EM_algorithm(observations = obs_matrix, 
                           dynamics_param_0 = true_ar_parameters, 
                           transition_mat_0 = transition_matrix, n_iter = 1)
estimation$lls %>% 
