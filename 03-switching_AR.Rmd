...
\begin{align*}
\frac{\sum_{t = 1}^n p(z_{t-1} = k, z_{t} = k'\vert y_{0:n}, \theta^{(0)})}{\mathbf{P}_{k,k'}} &= \lambda \\
\sum_{k' = 1}^K{\mathbf{P}_{kk'}} &= 1.
\end{align*}
This leads directly to:
\begin{align*}
\lambda &= \sum_{k' = 1}^K p(z_{t - 1} = k, z_{t} = k'\vert y_{0:n}, \theta^{(0)}) \\
\mathbf{P}_{kk'} &= \frac{p(z_{t - 1} = k, z_{t} = k'\vert y_{0:n}, \theta^{(0)}) }{\sum_{k' = 1}^K p(z_{t - 1} = k, z_{t} = k'\vert y_{0:n}, \theta^{(0)}) }
\end{align*}
